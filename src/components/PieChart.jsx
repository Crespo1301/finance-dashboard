...
function PieChart({
  ...
  const comparisonActive = hasCurrentPeriodData && hasPreviousPeriodData

  const currentExpenses = hasCurrentPeriodData ? periodExpenses : allExpenses
  ...
  Object.entries(currentTotals).forEach(([cat, value]) => {
    if (value / totalCurrent < OTHER_THRESHOLD) other += value
    else grouped[cat] = value
  })
  ...
  const centerValue = activeCategory ? grouped[activeCategory] : totalCurrent
  const centerDelta =
    comparisonActive && activeCategory && deltas[activeCategory] != null
      ? deltas[activeCategory]
      : null

  const getStatusMessage = () => {
    if (comparisonActive) return 'Comparing selected periods'
    ...
    <div className="p-6 sm:p-8 rounded-2xl bg-neutral-800 space-y-6">
      ...
      <div className="relative flex justify-center">
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-sm text-neutral-400">{activeCategory || 'Total'}</span>
          ...
          {centerDelta != null && (
            <span
              className={`text-xs mt-1 ${centerDelta >= 0 ? 'text-green-400' : 'text-red-400'}`}
            >
      ...
      <div className="text-xs text-neutral-500">{getStatusMessage()}</div>
    </div>
  )
}
